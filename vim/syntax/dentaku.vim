...
exe "syn match dtField /" . ident . "[:]" . ident . "/"
exe "syn match dtAnnot /++\\?" . ident . "/"
" exe "syn match dtDotted /[.][\\/]\\?" . ident . "/"

syn match dtNumber /[$]\?\d[0-9,]*\(\.\d\+\)\?\>/
syntax match dtKeyword /\c\<\(case\|when\|if\|in\|when\|then\|else\|end\|default\|missing\)\>/
syn match dtBool /\<true\>/
syn match dtBool /\<false\>/
...
hi! def link dtName NONE | hi! def link dtName        NONE
...
hi! def link dtNumber NONE | hi! def link dtNumber      Constant
hi! def link dtBool NONE | hi! def link dtBool      Constant
hi! def link dtField NONE | hi! def link dtField      Identifier
